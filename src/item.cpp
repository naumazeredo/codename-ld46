...
void update_position(u32 id, Point position) {
  if(item_info.items.find(id) == item_info.items.end());

  Item &item = item_info.items[id];

  position.x -= item_info.models[item.model].w/2;
...
u32 create_item(u32 model, Point position) {
  ...
  position.x -= item_info.models[model].w/2;
  item.position = position;
...
void render() {
  for(auto p: item_info.items) {
    ...
    render::add_to_render(item.position.x, item.position.y, model.w, model.h, model.texture);
  }
}
